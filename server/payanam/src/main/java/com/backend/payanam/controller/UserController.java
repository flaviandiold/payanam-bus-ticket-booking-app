...
import com.backend.payanam.service.UserService;
...
public class UserController {
	...
	public ResponseEntity<StoppingResponse> getStoppings(@PathVariable("bus_id") UUID busId){
		List<String> stoppings = service.getStoppingsOf(busId);
		if(stoppings == null) {
	...
	public ResponseEntity<TicketResponse> bookTicket(@RequestBody TicketRequest req, HttpServletRequest request){
		TicketResponse res = service.bookTicket(Integer.valueOf(service.getSubject(request.getHeader("Authorization").split(" ")[1])), req.busId(), req.from(), req.to());
		if(res == null) {
	...
	public ResponseEntity<Page<TicketsDTO>> allTickets(@PathVariable("page") Integer pageNumber, HttpServletRequest req){
		...
		return ResponseEntity.ok(service.getAllTickets(Integer.valueOf(service.getSubject(req.getHeader("Authorization").split(" ")[1])), pageRequest));
	}
